...
-- with subquery
select count(M1.schedule_id), S1.stadium_name, S1.city from 
(select IpL1.schedule_id, stadium_id from IPL_match_schedule IPL1 where status like 'completed' group by stadium_id, IPL1.schedule_id) M1 join IPL_stadium S1 on 
M1.stadium_id = S1.stadium_id group by S1.stadium_name, s1.city;
...
SELECT 
    ...
    ROUND(
        (SUM(CASE 
             ...
             END) 
        / COUNT(*)) * 100, 2
    ) AS toss_and_macth_win_percentage
FROM ipl_Match m
INNER JOIN ipl_Match_Schedule ms
    ON m.match_Id = ms.match_Id
WHERE ms.stadium_Id = 6   -- Replace with required stadiumId
...
select match_id, (case when team_id1 = match_winner then team_id1 else team_id2 end) as winner_team,  win_details 
from ipl_match;
...
SELECT 
    ...
    SUM(CASE 
            WHEN m.match_winner IS NOT NULL 
                 AND m.match_winner != t.team_id THEN 1 
            ELSE 0 
...
join ipl_match_schedule s on s.match_id = m.match_id
WHERE s.status = 'completed'
GROUP BY t.team_name
ORDER BY total_matches_played DESC;

-- 7.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order.

select p.team_id, count(p.player_role) as all_rounder 
from ipl_team_players P join ipl_team T on p.Team_id=t.team_id
where player_role ='All-Rounder'  
group by p.team_id having all_rounder >4 
